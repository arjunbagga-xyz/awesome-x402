...
- [StillOS Agent Notary](https://nolawealthfinancial.com/notary) - Signed, fail-closed verification, screening, and proprietary research primitives for the agent economy: `claim-verdict` commits a claim pre-resolution and grades it against external ground truth (GitHub PRs, Kalshi markets, HTTP probes); `grade-strategy` grades a settled trading track record into REAL_EDGE/REGIME_LUCK/NEGATIVE_EV/INSUFFICIENT_DATA; `screen-entity` checks a name against the OFAC SDN sanctions list; `regulatory-rules` and `federal-awards` search cached Federal Register / USAspending.gov data; `insider-conviction` and `smart-money` serve proprietary scoring over public EDGAR filings (insider cluster-buy conviction, 13D activist x insider composite); `distress-score` grades a single ticker live against a backtested distress model (modified Altman Z-score from SEC XBRL, 71% sensitivity / 100% specificity / ~109-day median lead on real Chapter 11 filings). Every response is Ed25519-signed and independently verifiable, no account needed. Free tier 3-5 calls/agent/day per endpoint, no signup; past that $0.02-$0.25 via x402 USDC on Base (CDP facilitator). ([Manifest](https://nolawealthfinancial.com/notary/.well-known/x402.json))
