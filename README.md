...
- [StillOS Agent Notary](https://nolawealthfinancial.com/notary) - Signed, fail-closed verification and screening primitives for the agent economy: `claim-verdict` commits a claim pre-resolution and grades it against external ground truth (GitHub PRs, Kalshi markets, HTTP probes); `grade-strategy` grades a settled trading track record into REAL_EDGE/REGIME_LUCK/NEGATIVE_EV/INSUFFICIENT_DATA; `screen-entity` checks a name against the OFAC SDN sanctions list; `regulatory-rules` searches recent Federal Register rule changes by agency/keyword; `federal-awards` searches the top-100-by-amount federal contract award snapshot by recipient. Every response is Ed25519-signed and independently verifiable, no account needed. Free tier 5 calls/agent/day per endpoint, no signup; past that $0.02-$0.25 via x402 USDC on Base (CDP facilitator). ([Manifest](https://nolawealthfinancial.com/notary/.well-known/x402.json))
