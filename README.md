...
- [Gate402](https://gate402.app) - x402 reverse-proxy payment gateway for M2M agent APIs. Unified `/v1/*` storefront fronting four pay-per-call services: web→Markdown scraping ($0.002, plus a $0.05 stealth tier that bypasses Cloudflare/Akamai), token compression that cuts upstream LLM bills ~40% ($0.005/10k tokens), and a semantic vector cache ($0.001 hit / $0.003 miss). Multi-network USDC (Base/Polygon/Arbitrum/Solana) via the Coinbase CDP facilitator, x402 **or** a non-crypto API-key/Stripe rail, machine-readable discovery + catalog, no signup. ([Discovery](https://gate402.app/.well-known/x402.json)) ([Catalog](https://gate402.app/v1)) ([MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=io.github.pgalyen1987))
