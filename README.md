...
- [almanac](https://almanac.locomot.io) - Deterministic physical-truth data for AI agents — the sky and the Earth's magnetic field, two things LLMs reliably hallucinate. `GET /sky` ($0.003): Sun/Moon/planet alt-az + distance, rise/set/transit, twilight, moon phase, and ecliptic longitudes for any lat/lon/time (JPL DE421, ~1 arcsec). `GET /field` ($0.002): magnetic declination, inclination, intensity, X/Y/Z components, and secular variation for any lat/lon/alt/date (World Magnetic Model 2025). Same input → same bytes; verified against NOAA's published WMM test values (100/100) and an independent ephemeris engine, so any agent can re-execute an answer and check it itself. USDC on Base via Coinbase CDP facilitator; x402 v2 with Bazaar discovery on every 402. No API keys, no signup; free schema + samples at `/`, `/sky/example`, `/field/example`. ([free library on PyPI](https://pypi.org/project/almanac-compute/)) ([GitHub](https://github.com/savecharlie/almanac))
- 
